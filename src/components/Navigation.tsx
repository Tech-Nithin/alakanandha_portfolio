...
const navItems: NavItem[] = [
  ...
  { label: "Education", href: "#education", number: "04" },
      ...
      <AnimatePresence>
        {open && (
          ...
          >
            ...
            {/* Nav Links */}
            <nav className="flex flex-col gap-0 overflow-y-auto max-h-[65vh] pr-2 scrollbar-none">
              {navItems.map((item, i) => (
                <div
                  key={item.label}
                  className="overflow-hidden border-b border-white/15 py-2 md:py-2.5"
                >
                  <motion.a
                    href={item.href}
                  ...
                  >
                    <span className="text-3xl md:text-5xl lg:text-6xl font-semibold text-white uppercase tracking-tight leading-none group-hover:translate-x-3 transition-transform duration-300 ease-out">
                      {item.label}
                    </span>
                    <span className="text-xs text-white/45 font-mono tracking-widest self-start mt-2">
                      {item.number}
                    </span>
                  </motion.a>
                </div>
